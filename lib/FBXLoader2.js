...
module.exports = ( function () {
	...
	Object.assign( THREE.FBXLoader.prototype, {
		...
		load: function ( url, onLoad, onProgress, onError ) {
			...
			loader.load( url, function ( buffer ) {
				...
				try {
					...
					var scene = self.parse( new Uint8Array( buffer ), resourceDirectory );
		 ...
		 * @param {Uint8Array} FBXData - Contents of FBX file to parse.
		 * @param {string} resourceDirectory - Directory to load external assets (e.g. textures ) from.
		 * @returns {THREE.Group}
		 */
		parse: function ( FBXData, resourceDirectory ) {

			var FBXTree;

			if ( isFbxFormatBinary( FBXData ) ) {

				FBXTree = new BinaryParser().parse( FBXData.buffer );

			} else {

				var FBXText = convertUint8ArrayToString( FBXData );
	...
	function parseImage( videoNode ) {
		...
		switch ( extension ) {
			...
			default:
				...
				console.warn( 'FBXLoader: No support image type ' + type );
				return;
	...
	function parseTexture( textureNode, loader, imageMap, connections ) {
		...
		var image = children !== undefined && children.length > 0 ? imageMap.get( children[ 0 ].ID ) : undefined;
		var currentPath = loader.path;

		if ( image ) {

			fileName = image;console.log(fileName);
		...
		if ( image ) loader.setPath( currentPath );
	...
	function genGeometry( geometryNode, deformer ) {
		...
		for ( var polygonVertexIndex = 0; polygonVertexIndex < indexBuffer.length; polygonVertexIndex ++ ) {
			...
			if ( endOfFace ) {
				...
				var face = new Face();
				var materials = materialInfo === undefined ? [ 0 ] : getData( polygonVertexIndex, polygonIndex, vertexIndex, materialInfo );
			...
			for ( var i = 0; i < conns.parents.length; ++ i ) {
				...
				for ( var FBX_ID in deformers ) {
					...
					if ( subDeformer ) {
						...
						if ( model2 && model2.isBone ) model.add( model2 );
	...
	function parseAnimations( FBXTree, connections, sceneGraph ) {
		...
		for ( nodeID in rawCurves ) {
			...
			if ( nodeID.match( /\d+/ ) ) {
				...
				if ( connections.get( animationCurve.id ) === undefined ) continue;
		...
		tmpMap.forEach( function ( curveNode ) {
			...
			if ( curveNode.attr === 'R' && curveNode.curves.x ) {
	...
	function parseAnimationNode( FBXTree, animationCurveNode, connections, sceneGraph ) {
		...
		var returnObject = {
			 ...
			 * @type {string}
			 */
			attr: animationCurveNode.attrName[ 0 ],
	...
	Object.assign( BinaryParser.prototype, {
		...
		parseProperty: function ( reader ) {
			...
			switch ( type ) {
				...
				case 'f':
					...
					if ( encoding === 0 ) return reader.getFloat32Array( arrayLength );
				...
				case 'd':
					...
					if ( encoding === 0 ) return reader.getFloat64Array( arrayLength );
				...
				case 'l':
					...
					if ( encoding === 0 ) return reader.getInt64Array( arrayLength );
				...
				case 'i':
					...
					if ( encoding === 0 ) return reader.getInt32Array( arrayLength );
				...
				case 'b':
					...
					if ( encoding === 0 ) return reader.getBooleanArray( arrayLength );
				...
				case 'S':
					...
					return reader.getStrings( length );
		...
		parseNode: function ( reader ) {

			var startOffset = reader.getOffset();
			var endOffset = reader.getUint32();
			var numProperties = reader.getUint32();
			var propertyListLen = reader.getUint32();
			var nameLen = reader.getUint8();
			var name = reader.getStrings( nameLen );
			...
			while ( endOffset > reader.getOffset() ) {
				...
				var node = this.parseNode( reader );
				...
				if ( node !== null ) {
					...
					if ( node.name.match( /^Properties\d+$/ ) ) {
						...
						for ( var i = 0, il = keys.length; i < il; i ++ ) {
							...
							if ( key.indexOf( 'Lcl ' ) >= 0 ) {

								properties[ key.replace( 'Lcl ', 'Lcl_' ) ] = properties[ key ];
								properties[ key.replace( 'Lcl ', 'Lcl_' ) ].value = properties[ key.replace( 'Lcl ', 'Lcl_' ) ].value.toString();
					...
					if ( node.name === 'P' ) {
						...
						var innerPropName = node.properties[ 0 ];
						var innerPropType1 = node.properties[ 1 ];
						var innerPropType2 = node.properties[ 2 ];
						var innerPropFlag = node.properties[ 3 ];
						var innerPropValue;

						if ( innerPropType1 === 'ColorRGB' || innerPropType1 === 'Vector3D' || innerPropType1.indexOf( 'Lcl ' ) === 0 ) {
							...
							innerPropValue = node.properties[ 4 ];

						}

						if ( innerPropValue === undefined ) innerPropValue = innerPropFlag;
						...
						properties.connections.push( node.properties );

					} else if ( Object.keys( node.properties ).length === 1 &&
							Object.keys( node.subNodes ).length === 0 ) {

						var key = Object.keys( node.properties )[ 0 ];
						...
						if ( subNodes[ node.name ] === undefined ) {
							...
							if ( node.id === '' ) {
								...
								if ( ! Array.isArray( subNodes[ node.name ] ) ) {
									...
									var tmp = subNodes[ node.name ];
									subNodes[ node.name ] = [];
									subNodes[ node.name ].push( tmp );
								...
								} else if ( Array.isArray( subNodes[ node.name ][ node.id ] ) ) {

									subNodes[ node.name ][ node.id ].push( node );

								} else {

									var tmp = subNodes[ node.name ][ node.id ];
									subNodes[ node.name ][ node.id ] = [];
									subNodes[ node.name ][ node.id ].push( tmp );
									subNodes[ node.name ][ node.id ].push( node );
			...
			if ( true || name === 'AnimationLayer' || ( propertyList.length > 0 && hasSubNode ) ) {

				if ( propertyList.length > 0 ) attrId = propertyList[ 0 ];
				if ( propertyList.length > 1 ) attrName = propertyList[ 1 ];
				if ( propertyList.length > 2 ) attrType = propertyList[ 2 ];

			}

			if ( properties.Node !== undefined ) { attrId = properties.Node; }

			if ( propertyList.length === 1 ) {

				properties[ 0 ] = propertyList[ 0 ];

			}

			if ( name === 'C' ) {

				for ( var i = 1, il = propertyList.length; i < il; i ++ ) {

					properties[ i - 1 ] = propertyList[ i ];
			...
			if ( name === 'P' ) {
				...
				for ( var i = 0; i < Math.max( propertyList.length, 5 ); i ++ ) {
		...
		parse: function ( buffer ) {
			...
			var reader = new BinaryReader( buffer, offset );
			...
			while ( reader.getOffset() + 11 * 16 < reader.size() - 1 ) {
	...
	Object.assign( BinaryReader.prototype, {
		...
		getInt64: function () {
			...
			return this.getUint32() + this.getUint32() * 0x100000000;
...
*/
	 ...
	 * @returns {boolean}
	 */
	function isFbxFormatBinary( array ) {

		var CORRECT = 'Kaydara FBX Binary  \0';

		return array.length >= CORRECT.length && CORRECT === convertUint8ArrayToString( array, 0, CORRECT.length );
	...
	function convertUint8ArrayToString( array, from, to ) {
